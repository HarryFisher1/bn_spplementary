...
description: |
  Quick introduction to Bayesian Networks which formed the basis of my PhD research
author:
  - name: Harry Fisher 
...
In traditional sports injury research, statistical analysis techniques such as correlational analysis and logistic regression are commonly used examine the relationship between predictor variables and injury occurrence. 
While easily accessible to most researchers, these techniques have been limited in their ability to successfully identify predictive factors, and are unable to fully account the complex and multifactorial nature of injury occurrence due to their linear and reductionist assumptions.
...
Given the exponential relationship with the number of parent node states and CPTs in a BN, care should be taken when decretizing variables, as a large number of states will results in an exponentially large number of model parameters, potentially making the model untractable [@Chen2012].
...
Figure \@ref(fig:nlehigh) shows the change in probability of being injured when NLE (A) and both NLE and stiffness (B) are fixed to their "High" states.
High major life events has the greatest increase on the probability of being injury in the example. High stiffness also increases probability of being injury by a further 8%. 

```{r nlehigh, out.width="100%", fig.pos="H", fig.cap="NLE fixed to High", fig.align='center', fig.cap ='Network with NLE (A) and both NLE and Stiffness (B) set to \"High\"'}
include_graphics("figs/measures/samiams/network4_5.png")
```


In contrast, figure \@ref(fig:nlelow) shows the change in probability of being injured when NLE (A) and both NLE and stiffness (B) are fixed to their "Low" states. With "Low" NLE, the probability of injury has signficantly decreased to only 18%. However when stiffness is also set to low, probability of being injured increases by 20% indicating that "Low" stiffness may actually increase probability of being injured, and there is an optimal level of stiffness required to remain healthy (figure \@ref(fig:nlelow)).  

```{r nlelow, out.width="100%", fig.cap='Networks with NLE (A) and NLE and Stiffness (B) set to \"Low\"', fig.pos="H", fig.align='center'}
include_graphics("figs/measures/samiams/network8_9.png") 
```
